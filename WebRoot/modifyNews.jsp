...
<body>
	...
	<div class="main">
		...
		<form name="addUserForm" method="post" action="news/Newsupdate">
			<input type="hidden" name="news.news_Id" value="${news.news_Id }" />
			<div class="main_input">
				<table>
					<tr>
						<th align=center colspan=16 style="height: 23px">修改新闻</th>
					</tr>
					<tr bgcolor="#DEE5FA">
					...
					<tr>
						...
						<td width="31%"><input type="text" name="news.newsTitle"
							value="${news.newsTitle }" class="text"><span> *</span></td>
						<td width="11%" class="tdbg1">所属栏目：</td>
						<td width="31%"><select name="news.category">
								<s:iterator value="categs">
									<c:if test="${categ_Id == category}">
										<option value="${categ_Id }" selected="selected">${categName
											}</option>
									</c:if>
									<c:if test="${categ_Id != category}">
										<option value="${categ_Id }">${categName }</option>
									</c:if>
								</s:iterator>
						</select><span> *</span></td>
					</tr>
					<tr>
						...
						<td colspan="3"><textarea rows="10" cols="150"
								name="news.newsContent">${news.newsContent }</textarea><span>
								*</span></td>
