...
<body>
	...
	<div class="main">
		<div class="title">系统首页&gt;&gt;管理角色&gt;&gt;添加角色权限</div>
		<div id="layout_multiSelect">
			<form name="userRoleUpdateForm" method="post"
				action="role/RoleaddAuth">
				<input type="hidden" name="id" value="${id }" />
				<table>
					<tr>
						<th align=center colspan=16 style="height: 23px">添加角色权限</th>
					</tr>
					<tr bgcolor="#DEE5FA">
					...
					<tr>
						<td width="45%"><select name="allRoleIds" multiple="multiple"
							id="select_left" class="multiSelect_left">
								<s:iterator value="auths" var="auth">
									...
									<s:iterator value="role.auths" var="roleAuth" status="st">
										<c:set value="${st.count }" var="count"></c:set>
										<c:if test="${auth.auth_Id != roleAuth.auth_Id }">
						...
						<td width="45%"><select name="auths" multiple="multiple"
							id="select_right" class="multiSelect_left">
								<s:iterator value="role.auths" var="roleAuth">
